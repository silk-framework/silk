...
class ActivityClient(baseUrl: String, projectId: Identifier, taskId: Identifier)(implicit app: Application) {
  ...
  def startBlocking(activityId: String, parameters: Map[String, String] = Map.empty): Unit = {
    val startActivityRequest = WS.url(s"$activities/$activityId/startBlocking")
    checkResponse(startActivityRequest.post(parameters map { case (k, v) => (k, Seq(v)) }))
  }

  def activityValue(activityId: String): JsValue = {
  ...
  def activityStatus(activityId: String): JsValue = {
    val getActivityStatusRequest = WS.url(s"$activities/$activityId/status")
