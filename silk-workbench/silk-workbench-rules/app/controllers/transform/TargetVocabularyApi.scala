...
class TargetVocabularyApi extends Controller with ControllerUtilsTrait {
  ...
  def getTypeInfo(projectName: String, transformTaskName: String, typeUri: String): Action[AnyContent] = Action { implicit request =>
    ...
    vocabularies.findClass(fullTypeUri.uri) match {
      ...
      case None =>
        ErrorResult.requestError(NotFoundException(s"Type $typeUri could not be found in any of the target vocabularies."))
    }
  }
  ...
  def getPropertyInfo(projectName: String, transformTaskName: String, propertyUri: String): Action[AnyContent] = Action { implicit request =>
    ...
    vocabularies.findProperty(fullPropertyUri.uri) match {
      ...
      case None =>
        ErrorResult.requestError(NotFoundException(s"Property $propertyUri could not be found in any of the target vocabularies."))
    }
  }
  ...
  def getTypeOrPropertyInfo(projectName: String, transformTaskName: String, uri: String): Action[AnyContent] = Action { implicit request =>
    ...
    (vocabularies.findClass(fullUri.uri), vocabularies.findProperty(fullUri.uri)) match {
      ...
      case (None, Some(vocabProperty)) =>
        serializeCompileTime(vocabProperty)
      case (None, None) =>
        ErrorResult.requestError(NotFoundException(s"Property or Class $uri could not be found in any of the target vocabularies."))
      case (Some(_), Some(_)) =>
