...
function addTypeSelections(typeSelects) {
    var types = [{ label: 'Autodetect', value: 'AutoDetectValueType', category: '' }, { label: 'Resource', value: 'UriValueType', category: '' }, { label: 'Boolean', value: 'BooleanValueType', category: 'Literals' }, { label: 'String', value: 'StringValueType', category: 'Literals' }, {
        label: 'Integer',
...
var createSourcePathElement = function createSourcePathElement(path) {
    ...
    if (pathIndex > 0) {
        $(element).addClass('di-rule-example-path-' + mapToRange(pathIndex, maxPathNumber));
    }
    if (settings.class) {
...
var createValueElement = function createValueElement(value) {
    ...
    $(chip).attr('id', chipId);
    if (pathIndex > 0) {
        $(chip).addClass('di-rule-example-path-' + mapToRange(pathIndex, maxPathNumber));
    }
    if (settings.class) {
