...
/**
  ...
  */
case class TypedProperty(propertyUri: Uri, valueType: ValueType, isBackwardProperty: Boolean = false, isAttribute: Boolean = false) {

  /**
   * Tests if this is the (RDF) type property as generated by a type mapping.
   */
  def isTypeProperty: Boolean = {
    propertyUri.uri == TypedProperty.rdfTypeUri
  }

  /**
   * Tests if this is the column that holds the entity URI.
   */
  def isUriProperty(datasetSpec: DatasetSpec[_]): Boolean = {
...
object TypedProperty {
  ...
  private val rdfTypeUri = "http://www.w3.org/1999/02/22-rdf-syntax-ns#type"

  def rdfTypeProperty: TypedProperty = {
    TypedProperty(rdfTypeUri, ValueType.URI)
  }
