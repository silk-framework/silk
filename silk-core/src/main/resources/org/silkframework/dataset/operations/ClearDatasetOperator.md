...
- **Clear before write (full load):** place the target dataset twice on the canvas — one node fed by this operator (clear role), one node fed by the data-producing tasks (write role). Connect **every** writing node to the clear node with a *dependency* connection, so all writes run after the clear. A dataset can be cleared multiple times in one workflow (e.g. to reuse it freshly in a later phase) — but then order **each** clear node against all writes the same way, or a later clear may silently remove earlier writes.
