...
class EqualityMetricTest extends FlatSpec with ShouldMatchers
{
  ...
  "EqualityMetric" should "return 0.0 if the string differ" in
  {
    ...
    metric.evaluate("123", "124", t) should be (approximatelyEqualTo (0.0))
    metric.evaluate("", "xxx", t) should be (approximatelyEqualTo (0.0))
  }

  "EqualityMetric" should "return 1.0 if the string are equal" in
  {
    metric.evaluate("aaa", "aaa", t) should be (approximatelyEqualTo (1.0))
    metric.evaluate("  _", "  _", t) should be (approximatelyEqualTo (1.0))
    metric.evaluate("123", "123", t) should be (approximatelyEqualTo (1.0))
  }
}