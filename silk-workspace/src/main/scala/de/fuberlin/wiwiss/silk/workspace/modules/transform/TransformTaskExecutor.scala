...
class TransformTaskExecutor extends TaskExecutor[TransformSpecification] {
  ...
  def apply(inputs: Seq[DataSource], transformSpec: TransformSpecification, outputs: Seq[Dataset]) = {
    require(inputs.size == 1, "Transform tasks expect exactly one input dataset.")

    val input = inputs.head
    val activity = new ExecuteTransform(input, transformSpec.selection, transformSpec.rules, outputs.map(_.sink))
    activity
  }
}
