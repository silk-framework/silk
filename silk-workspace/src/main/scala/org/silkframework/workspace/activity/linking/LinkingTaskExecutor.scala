...
class LinkingTaskExecutor extends TaskExecutor[LinkSpecification] {
  ...
  override def apply(inputs: Seq[DataSource], linkSpec: LinkSpecification, linkOutputs: Seq[LinkSink], entityOutputs: Seq[EntitySink]) = {
    ...
    new GenerateLinks(inputPair, linkSpec, linkOutputs)
  }
}
