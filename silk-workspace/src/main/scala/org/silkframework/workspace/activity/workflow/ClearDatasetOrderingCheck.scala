...
/**
  ...
  * executes; without an explicit order (a data/dependency path between the nodes) the clear may run
  * before or after the writes.
  */
object ClearDatasetOrderingCheck {
  ...
  /**
    ...
    * A pair is not reported if a (transitive) path connects the two nodes in either direction, or if
    * clear and write happen on the same node (port order decides there). May throw if the workflow
    * graph is invalid (e.g. cyclic).
    */
  ...
  private def unorderedPairs(workflow: Workflow, project: Project, clearOperatorNodeIds: Set[String])
    ...
    workflow.workflowDependencyGraph
