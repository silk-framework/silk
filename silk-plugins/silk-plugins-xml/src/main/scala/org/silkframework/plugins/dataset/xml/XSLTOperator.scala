...
@Plugin(
  ...
  documentationFile = "XSLTOperator.md"
)
case class XSLTOperator(
  @Param(
    value = "The XSLT file to be used for transforming XML.",
    autoCompletionProvider = classOf[ResourceAutoCompletionProvider],
  ...
  override lazy val outputPort: Option[Port] = Some(FixedSchemaPort(FileEntitySchema.schema))
