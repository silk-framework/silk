...
case class YearsDistanceMetric(blockingParameter: Double = 1.0) extends SingleValueDistanceMeasure {
  ...
  override def indexValue(str: String, distance: Double, sourceOrTarget: Boolean): Index = {
    Utils.indexTimes(str, blockingParameter, distance, Constants.YEARS_DISTANCE)
  }
}