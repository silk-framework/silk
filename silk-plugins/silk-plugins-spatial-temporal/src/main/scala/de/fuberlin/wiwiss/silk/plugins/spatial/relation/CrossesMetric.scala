...
@Plugin(
  ...
  label = "Crosses",
  description = "Computes the relation \"crosses\" between two geometries.")
case class CrossesMetric(blockingParameter: Double = 1.0) extends SingleValueDistanceMeasure {

  override def evaluate(str1: String, str2: String, limit: Double): Double = {
    Utils.evaluateRelation(str1, str2, limit, Constants.CROSSES)
  }
