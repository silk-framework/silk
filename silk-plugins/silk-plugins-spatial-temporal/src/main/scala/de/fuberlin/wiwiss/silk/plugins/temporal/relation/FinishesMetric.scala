...
@Plugin(
  ...
  label = "Finishes",
  description = "Computes the relation \"finishes\" between two time periods or instants.")
case class FinishesMetric(blockingParameter: Double = 1.0) extends SingleValueDistanceMeasure {

  override def evaluate(str1: String, str2: String, limit: Double): Double = {
    Utils.evaluateRelation(str1, str2, Constants.FINISHES)
  }
