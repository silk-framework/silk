...
//    Map(EntityMetadataLegacy.FAILURE_KEY -> LazyMetadataJson(testException.failureClass, FailureClassSerializerJson()))
//  )
//
...
//    val lazyMetadata = metadata.getLazyMetadata[FailureClass](EntityMetadataLegacy.FAILURE_KEY)
...
//      Map(
//        EntityMetadataLegacy.FAILURE_KEY -> LazyMetadataJson(FailureClass(GenericExecutionFailure(testException), alibiTask.id), FailureClassSerializerJson()),
...
//    val ex = readMetadata.getLazyMetadata[FailureClass](EntityMetadataLegacy.FAILURE_KEY).metadata.get
