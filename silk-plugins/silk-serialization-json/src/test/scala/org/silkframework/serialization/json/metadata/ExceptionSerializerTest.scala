...
class ExceptionSerializerTest extends FlatSpec with Matchers {
  ...
  it should "not fail on exceptions without certain features occur in exception objects (missing string constructor, null values)" in {
    ...
    val ex1 = new NoConstructorException(cau) // see CMEM-1472, lead to trouble with some of Sparks exceptions
...
//    val jsv =  play.api.libs.json.Json.parse(exceptionJSon)
    ...
    try {
      val nde = ser.write(jsv)(ReadContext())
      val res = ser.read(nde)(ReadContext())
      Some(res)
    }
    catch {
...
case class NoConstructorException(ex: Throwable) extends Throwable
