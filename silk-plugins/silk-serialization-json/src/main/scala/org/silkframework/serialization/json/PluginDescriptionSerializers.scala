  ...
  implicit object PluginListJsonFormat extends WriteOnlyJsonFormat[PluginList] {
    ...
    private def serializeParam(param: PluginParameter,
      ...
      Json.toJson(PluginParameterJson(
        ...
        // The value format of non-obvious parameter types (e.g. stringmap: "Key1:Value1,Key2:Value2");
        parameterTypeDescription = Some(param.parameterType.description).filter(_ != param.parameterType.name),
        value = defaultValue,
...
case class PluginParameterJson(name: String,
                               ...
                               parameterTypeDescription: Option[String],
                               value: JsValue,
                               advanced: Boolean,
