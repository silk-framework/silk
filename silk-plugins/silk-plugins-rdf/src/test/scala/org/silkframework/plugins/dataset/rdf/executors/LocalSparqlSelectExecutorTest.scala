...
class LocalSparqlSelectExecutorTest extends AnyFlatSpec
  ...
  it should "not run out of memory and fetch first entity immediately on large result sets" in {
    ...
    val entities = new LocalSparqlSelectIterator(task, sparqlEndpoint, task.selectQuery.str, executionReportUpdater = Some(reportUpdater))
  ...
  it should "pass query timeout to SPARQL endpoint if a query timeout is configured" in {
    ...
    val entities = new LocalSparqlSelectIterator(task, sparqlEndpoint, task.selectQuery.str, limit, Some(reportUpdater))
  ...
  it should "evaluate a Jinja query template using the graph variable from the task parameters" in {
    ...
    val generatedQuery = LocalSparqlSelectExecutor().generateQuery(task, taskWithEndpoint(graphUri = Some(graphUri)), None)

    task.outputSchema.typedPaths.map(_.toUntypedPath.normalizedSerialization) mustBe IndexedSeq("s", "p", "o")
    generatedQuery must include(s"<$graphUri>")
    generatedQuery must not include "inputProperties.uri"
  }
  ...
  private def taskWithEndpoint(sparqlEndpoint: SparqlEndpoint = sparqlEndpointStub(),
                               graphUri: Option[String] = None): Task[DatasetSpec[RdfDataset]] = {
