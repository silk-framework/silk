...
const adaptWebpackConfig = (config) => {
    ...
    if (paths.additionalSourcePaths().length > 0 || paths.additionalEntries().length > 0) {
        paths.additionalEntries().length > 0 &&
            console.log("Add additional entry point/s: " + paths.additionalEntries().join(", "));
        adaptedConf.entry = adaptedConf.entry.concat(paths.additionalEntries());
        paths.additionalSourcePaths().length > 0 &&
            adaptedConf.module.rules.forEach((rule) => {
