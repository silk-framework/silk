import { Grid, GridColumn, GridRow, TestableComponent } from "@eccenca/gui-elements";
...
export const ComparisonDataHeader = ({ children, className = "" }: ComparisonDataObjectProps) => {
    return <GridColumn className={`diapp-linking-learningdata__header ${className}`}>{children}</GridColumn>;
};

export const ComparisonDataCell = ({ children, className = "" }: ComparisonDataObjectProps) => {
    return <GridColumn className={`diapp-linking-learningdata__cell ${className}`}>{children}</GridColumn>;
};
