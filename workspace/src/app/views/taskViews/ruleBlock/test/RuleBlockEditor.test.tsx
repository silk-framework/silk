...
const renderRuleBlockEditor = async ({
    ...
    // Flush the mocked task-data promise chain (fetchRuleData -> applyLoadedRuleBlockTask ->
    // setPorts) and the re-render it schedules, so portsRef is populated before tests call
    // saveRule & co. With React 19 the re-render is scheduled later than under React 18, which
    // made tests that rely on the loaded ports racy without this explicit flush.
    await act(async () => {});
