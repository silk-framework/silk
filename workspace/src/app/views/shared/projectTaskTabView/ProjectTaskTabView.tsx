...
const OpenInNewTabIcon = () => <Icon name="navigation-extern" small aria-hidden="true" focusable={false} />;
...
export function ProjectTaskTabView({
    ...
    const externalLinkDescriptionId = `${tabsId}-external-link-description`;
    ...
    const inlineTabs = viewsAndItemLink.filter((view) => !view.openInNewTab);
    const showTabs = inlineTabs.length > 1;
    const openInNewTabExist = viewsAndItemLink.some((tabItem) => tabItem.openInNewTab)
    const selectedTabId = typeof selectedTab === "string" ? selectedTab : selectedTab?.id;
    // Match the IDs generated by Tabs for panels managed outside its header.
    const tabTitleId = (id: string) => `${ClassNames.Blueprint.TAB}-title_${tabsId}_${id}`;
    const tabPanelId = (id: string) => `${ClassNames.Blueprint.TAB_PANEL}_${tabsId}_${id}`;
    ...
    const changeTab = (tabItem: IItemLink | string, overwrite = false) => {
        if ((typeof tabItem === "string" ? tabItem : tabItem.id) === selectedTabId) {
            return;
        }
        if (unsavedChanges && !overwrite) {
            //trigger modal
        ...
        return (
            ...
            >
                ...
                >
                        ...
                        <CardOptions>
                            {showTabs && (
                                <Tabs
                                    ...
                                    onChange={(id) => changeTab(String(id))}
                                    allowScrollbars
                                    tabs={inlineTabs.map((tabItem) => ({
                                        id: tabItem.id ?? "",
                                        title: tLabel(tabItem.label ?? ""),
                                        "data-test-id": "taskView-" + tabItem.id,
                                    }))}
                                />
                            )}
                            {openInNewTabExist && (
                                <span id={externalLinkDescriptionId} hidden>
                                    {t("common.action.openInNewTabTooltip")}
                                </span>
                            )}
                            {viewsAndItemLink
                                .filter((tabItem) => tabItem.openInNewTab)
                                .map((tabItem) => (
                                    <Link
                                        data-test-id={"taskView-" + tabItem.id}
                                        key={tabItem.id}
                                        title={t("common.action.openInNewTabTooltip")}
                                        aria-label={tLabel(tabItem.label ?? "")}
                                        aria-describedby={externalLinkDescriptionId}
                                        renderIcon={OpenInNewTabIcon}
                                        href={tabItem.path}
                                        target="_blank"
                                    >
                            ...
                            {!!handlerRemoveModal ? (
                                ...
                                <IconButton
                                    ...
                                    text={t(displayFullscreen ? "common.action.minimize" : "common.action.maximize")}
                                    aria-label={t(
                                        displayFullscreen ? "common.action.minimize" : "common.action.maximize",
                                    )}
                                    tooltipAsTitle
                    ...
                    <Divider />
                    {/* Keep every aria-controls target present without mounting inactive editors. */}
                    {showTabs &&
                        inlineTabs
                            .filter((tabItem) => tabItem.id !== selectedTabId)
                            .map((tabItem) => (
                                <div
                                    key={tabItem.id}
                                    id={tabPanelId(tabItem.id ?? "")}
                                    role="tabpanel"
                                    aria-labelledby={tabTitleId(tabItem.id ?? "")}
                                    hidden
                                />
                            ))}
                    <CardContent
                        id={showTabs && selectedTabId ? tabPanelId(selectedTabId) : undefined}
                        role={showTabs ? "tabpanel" : undefined}
                        aria-labelledby={showTabs && selectedTabId ? tabTitleId(selectedTabId) : undefined}
