...
export function Header({
  ...
  const { dmBaseUrl, dmModuleLinks, version } = useSelector(
  ...
  const diUserMenuItems = pluginRegistry.pluginReactComponent<{}>(
    SUPPORTED_PLUGINS.DI_USER_MENU_ITEMS,
  );
  const diUserMenuFooter =
    pluginRegistry.pluginReactComponent<UserMenuFooterProps>(
      SUPPORTED_PLUGINS.DI_USER_MENU_FOOTER,
    );
  const languageSwitcher = pluginRegistry.pluginReactComponent<{}>(
    SUPPORTED_PLUGINS.DI_LANGUAGE_SWITCHER,
  );
      ...
      <ApplicationHeader aria-label={`${APPLICATION_NAME()}${brandingSuffix}`}>
        ...
        >
          {!!dmBaseUrl ? (
            <>
              <TitleSubsection
                title={t("navigation.side.dmBrowserTooltip", "")}
              >
                ...
                {dmModuleLinks ? (
                  dmModuleLinks.map((link) => (
                    <MenuItem
                      ...
                      text={t(
                        "navigation.side.dm." + link.path,
                        link.defaultLabel,
                      )}
                      htmlTitle={t(
                        "navigation.side.dm." + link.path + "Tooltip",
                      )}
          ...
          <Menu>
            <MenuItem
              ...
              active={
                location.pathname === SERVE_PATH &&
                locationParams.get("itemType") === "project"
              }
            />
            <MenuItem
              ...
              active={
                location.pathname === SERVE_PATH &&
                locationParams.get("itemType") === "dataset"
              }
            />
            <MenuItem
              ...
              active={
                location.pathname === SERVE_PATH &&
                locationParams.get("itemType") === "workflow"
              }
            />
            <MenuItem
                ...
                <Toolbar verticalStack={true} style={{ height: "100%" }}>
                  <ToolbarSection canGrow={true} style={{ width: "100%" }}>
                    <Menu>
                      ...
                      <MenuItem
                        text={t(
                          "common.action.listDeprecatedPlugins",
                          "Deprecated Plugins",
                        )}
