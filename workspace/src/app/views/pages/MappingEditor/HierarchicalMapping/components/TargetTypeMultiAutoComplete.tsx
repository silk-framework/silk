...
const TargetTypeMultiAutoComplete = ({ placeholder, className, isValidNewOption, value, onChange }: Props) => {
    const mappingEditorContext = React.useContext(GlobalMappingEditorContext);
    const runOnQueryChange = React.useCallback((query: string) => {
        return silkRestApi.targetClassAutoCompletions(mappingEditorContext.projectId, mappingEditorContext.transformTaskId, query, 50)
            .the
    }, [mappingEditorContext.projectId, mappingEditorContext.transformTaskId])
    return <MultiSuggestField<Omit<TargetPropertyAutoCompletion, "extra"> | string>
        className={className}
