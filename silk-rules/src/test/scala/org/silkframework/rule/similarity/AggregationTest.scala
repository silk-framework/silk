...
class AggregationTest extends AnyFlatSpec with Matchers {
  ...
  private def eval(ops: Seq[SimilarityOperator]) = {
    Aggregation(operators = ops, aggregator = aggregator).execution(TaskContext.empty).apply(null)
  }

  private def index(ops: Seq[SimilarityOperator]) = {
    Aggregation(operators = ops, aggregator = aggregator).execution(TaskContext.empty).index(null, true, 0.0)
  }
