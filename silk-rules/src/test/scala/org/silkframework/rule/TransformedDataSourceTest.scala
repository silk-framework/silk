...
class TransformedDataSourceTest extends FlatSpec with MustMatchers {
  ...
  it should "retrieve by URI on the transformed entities" in {
    ...
    csvDataset.source.retrieveByUri(entitySchema, entities = Seq(Uri(entityUris.head))) must have size 1
    // Transformed entities cannot be retrieved via source entity URIs
    transformedDataSource.retrieveByUri(entitySchema, entities = Seq(Uri(entityUris.head))) must have size 0
    // Transformed entities can be retrieved via transformed entity URIs
    transformedDataSource.retrieveByUri(entitySchema, entities = Seq(Uri(transformedUris.head))) must have size 1
  }
}
