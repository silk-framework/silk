...
object TransformSpec {
  ...
  private def extractTypedPaths(rule: TransformRule): IndexedSeq[TypedPath] = {
    ...
    val valuePaths = valueRules.flatMap(_.sourcePaths).map(p => TypedPath(p.operators, StringValueType, isAttribute = false)).distinct.toIndexedSeq
    val objectPaths = objectRulesWithDefaultPattern.flatMap(_.sourcePaths).map(p => TypedPath(p.operators, UriValueType, isAttribute = false)).distinct.toIndexedSeq
