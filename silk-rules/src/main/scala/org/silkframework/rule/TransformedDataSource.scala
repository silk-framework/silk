...
class TransformedDataSource(source: DataSource, inputSchema: EntitySchema, transformRule: TransformRule, task: Task[TransformSpec]) extends DataSource {
  ...
  /**
    ...
    * Falls back to this source's own rule if no rule generates the sub path.
    */
  private def ruleForSubPath(subPath: UntypedPath): (EntitySchema, TransformRule) = {
    if(subPath.operators.isEmpty) {
      ...
      val basePath = ruleSchemata.find(_.transformRule.id == transformRule.id).map(_.outputSchema.subPath).getOrElse(UntypedPath.empty)
      ruleSchemata.find(_.outputSchema.subPath == basePath ++ subPath) match {
        case Some(nestedRule) => (nestedRule.inputSchema, nestedRule.transformRule)
        case None => (inputSchema, transformRule)
      }
    }
  }
