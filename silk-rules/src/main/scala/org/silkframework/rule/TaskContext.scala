 ...
 */
case class TaskContext(inputTasks: Seq[Task[_ <: TaskSpec]],
...
object TaskContext {
   ...
   */
  def forInput(inputTask: Task[_ <: TaskSpec])
              (implicit pluginContext: PluginContext): TaskContext = {
   ...
   */
  def forInputs(inputTasks: Seq[Task[_ <: TaskSpec]])
               (implicit pluginContext: PluginContext): TaskContext = {
