...
private object MetaphoneAlgorithm {
    ...
    if (c == '\u0000' && r.length == 0) o
      ...
      val t = {
        (c: @annotation.switch) match {
          ...
          case 'h' =>
            if ((l.length >= 1 && (Alpha.isLowercaseVowel(l.last)) && (r.length == 0 || !(Alpha.isLowercaseVowel(r.head))))
              || (l.length >= 2 && l.last == 'h'
          ...
          case 'v' => shift(1, o :+ 'f')
          case 'w' | 'y' => if (r.length == 0 || !(Alpha.isLowercaseVowel(r.head))) shift(1, o) else shift(1, o :+ c)
