...
object LinkageRule {
  ...
  implicit object LinkageRuleFormat extends XmlFormat[LinkageRule] {
    ...
    def write(value: LinkageRule)(implicit writeContext: WriteContext[Node]): Node = {
      <LinkageRule linkType={value.linkType.serialize(writeContext.prefixes)} inverseLinkType={value.inverseLinkType.map(_.serialize(writeContext.prefixes)).getOrElse("")}>
