...
exists. Otherwise, the output is empty.
...
the condition, i.e. the output if the regex matches the first input value, and the alternative otherwise.
...
* On no match, then the _third_ input is returned, provided it exists.
* A missing third input returns an empty sequence.
