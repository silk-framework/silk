...
class CleanPopulationTask(population: Population, entities: ReferenceEntities, generator: LinkageRuleGenerator) extends Task[Population] {
  ...
  private def cleanAggregation(location: NodeTraverser)(implicit fitness: Double): NodeTraverser = location.node match {
    case AggregationNode(_, (operator: AggregationNode) :: Nil) =>  cleanAggregation(location.moveDown.get)
    case AggregationNode(_, operator :: Nil) => location.moveDown.get
    case AggregationNode(_, operators) => removeRedundantOperators(location, Nil, operators)
    case _ => location
  }
