 ...
 */
@Plugin(
  id = "MinDistanceMetric",
  categories = Array("Spatial"),
  label = "Min distance",
  description = "Computes the minimum distance between two geometries in meters. Author: Panayiotis Smeros (Department of Informatics & Telecommunications, National & Kapodistrian University of Athens)")
case class MinDistanceMetric() extends SimpleDistanceMeasure {

  override def evaluate(str1: String, str2: String, limit: Double): Double = {
    evaluateDistance(str1, str2, limit, MIN_DISTANCE)
  }
