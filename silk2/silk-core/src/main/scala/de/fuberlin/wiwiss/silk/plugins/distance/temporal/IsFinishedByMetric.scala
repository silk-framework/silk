...
/**
 * Computes the relation \"isFinishedBy\" between two time periods or instants (It assumes that the times are expressed in the "yyyy-MM-DD'T'hh:mm:ss" format).
 * @author Panayiotis Smeros (Department of Informatics & Telecommunications, National & Kapodistrian University of Athens)
 */
@Plugin(
  id = "IsFinishedByMetric",
  categories = Array("Temporal"),
  label = "IsFinishedBy",
  description = "Computes the relation \"isFinishedBy\" between two time periods or instants. Author: Panayiotis Smeros (Department of Informatics & Telecommunications, National & Kapodistrian University of Athens)")
case class IsFinishedByMetric() extends SimpleDistanceMeasure {
  ...
  override def evaluate(str1: String, str2: String, limit: Double): Double = {
    evaluateRelation(str1, str2, limit, IS_FINISHED_BY)
  }
