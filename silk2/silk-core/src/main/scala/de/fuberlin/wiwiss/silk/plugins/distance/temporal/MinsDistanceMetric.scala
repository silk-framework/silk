...
@Plugin(
  ...
  label = "Mins distance",
  description = "Computes the distance in minutes between two time periods or instants. Author: Panayiotis Smeros (Department of Informatics & Telecommunications, National & Kapodistrian University of Athens)")
case class MinsDistanceMetric() extends SimpleDistanceMeasure {
  ...
  override def evaluate(str1: String, str2: String, limit: Double): Double = {
    evaluateDistance(str1, str2, limit, MINS_DISTANCE)
  }

  override def indexValue(str: String, distance: Double): Index = {
    indexGeometries(str, distance*MILLISECS_PER_MIN)
  }
}