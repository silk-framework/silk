...
/**
 * Computes every relation from DE-9IM between two geometries (It assumes that geometries are expressed in WKT and WGS 84 (latitude-longitude)).
...
@Plugin(
  ...
  label = "Relate",
  description = "Computes every relation from DE-9IM between two geometries. Author: Panayiotis Smeros (Department of Informatics & Telecommunications, National & Kapodistrian University of Athens)")
case class RelateMetric(blockingParameter: Double = 1.0, relation: String = "") extends SimpleDistanceMeasure {
  ...
  override def evaluate(str1: String, str2: String, limit: Double): Double = {
    evaluateRelation(str1, str2, limit, relation)
  }
