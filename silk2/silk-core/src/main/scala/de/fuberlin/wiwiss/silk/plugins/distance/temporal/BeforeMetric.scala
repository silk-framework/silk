...
@Plugin(
  ...
  label = "Before",
  description = "Computes the relation \"before\" between two time periods or instants. Author: Panayiotis Smeros (Department of Informatics & Telecommunications, National & Kapodistrian University of Athens)")
case class BeforeMetric() extends SimpleDistanceMeasure {

  override def evaluate(str1: String, str2: String, limit: Double): Double = {
    evaluateRelation(str1, str2, limit, BEFORE)
  }
