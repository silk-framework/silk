...
/**
 * Computes the relation \"isMetBy\" between two time periods or instants (It assumes that the times are expressed in the "yyyy-MM-DD'T'hh:mm:ss" format).
...
@Plugin(
  ...
  label = "IsMetBy",
  description = "Computes the relation \"isMetBy\" between two time periods or instants. Author: Panayiotis Smeros (Department of Informatics & Telecommunications, National & Kapodistrian University of Athens)")
case class IsMetByMetric() extends SimpleDistanceMeasure {

  override def evaluate(str1: String, str2: String, limit: Double): Double = {
    evaluateRelation(str1, str2, limit, IS_MET_BY)
  }
