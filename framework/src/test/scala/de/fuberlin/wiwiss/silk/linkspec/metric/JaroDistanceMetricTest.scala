...
class JaroDistanceMetricTest extends FlatSpec with ShouldMatchers
{
    ...
    /*
    "JaroDistanceMetric" should "return distance 0.79" in
    {
        metric.evaluate("JONES", "JOHNSON") should be (approximatelyEqualTo (0.79))
        metric.evaluate("JOHNSON", "JONES") should be (approximatelyEqualTo (0.79))
    }
    */
