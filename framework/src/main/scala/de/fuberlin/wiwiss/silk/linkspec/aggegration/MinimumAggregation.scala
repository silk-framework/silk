...
class MinimumAggregation(val weight : Int, val operators : Traversable[Operator]) extends Aggregation
{
    override def evaluate(sourceInstance : Instance, targetInstance : Instance) =
    {
        ...
        if (values.isEmpty) Traversable() else Traversable(values.min)
    }
}